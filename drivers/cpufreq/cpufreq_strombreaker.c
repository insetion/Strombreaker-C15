/*
* drivers/cpufreq/cpufreq_strombreaker.c
*
...
static DEFINE_PER_CPU(struct strombreaker_info_s, strombreaker_info);
...
static int cpufreq_governor_strombreaker(struct cpufreq_policy *policy,
unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_strombreaker
static
#endif
struct cpufreq_governor cpufreq_gov_strombreaker = {
.name = "strombreaker",
.governor = cpufreq_governor_strombreaker,
.max_transition_latency = 6000000,
.owner = THIS_MODULE,
};

static void cpufreq_strombreaker_timer(unsigned long data)
{
u64 delta_idle;
u64 update_time;
u64 now_idle;
struct strombreaker_info_s *this_strombreaker = &per_cpu(strombreaker_info, data);
struct cpufreq_policy *policy = this_strombreaker->cur_policy;

now_idle = get_cpu_idle_time_us(data, &update_time);

if (update_time == this_strombreaker->idle_exit_time)
return;

delta_idle = cputime64_sub(now_idle, this_strombreaker->time_in_idle);
//printk(KERN_INFO "strombreaker: t=%llu i=%llu\n",cputime64_sub(update_time,this_strombreaker->idle_exit_time),delta_idle);
...
this_strombreaker->force_ramp_up = 1;
...
*/
if (!timer_pending(&this_strombreaker->timer) && nr_running() > 0) {
this_strombreaker->time_in_idle = get_cpu_idle_time_us(
data, &this_strombreaker->idle_exit_time);
mod_timer(&this_strombreaker->timer, jiffies + sample_rate_jiffies);
}
...
{
struct strombreaker_info_s *this_strombreaker = &per_cpu(strombreaker_info, smp_processor_id());
struct cpufreq_policy *policy = this_strombreaker->cur_policy;
...
if (timer_pending(&this_strombreaker->timer) == 0) {
this_strombreaker->time_in_idle = get_cpu_idle_time_us(
smp_processor_id(), &this_strombreaker->idle_exit_time);
mod_timer(&this_strombreaker->timer, jiffies + sample_rate_jiffies);
}
}
...
*/
static unsigned int cpufreq_strombreaker_calc_freq(unsigned int cpu, struct cpufreq_policy *policy)
{
...
//printk(KERN_INFO "strombreaker calc_freq: delta_time=%u cpu_load=%u\n",delta_time,cpu_load);
...
//printk(KERN_INFO "strombreaker calc_freq: %u => %u\n",policy->cur,new_freq);
return new_freq;
...
static void cpufreq_strombreaker_freq_change_time_work(struct work_struct *work)
{
unsigned int cpu;
unsigned int new_freq;
struct strombreaker_info_s *this_strombreaker;
struct cpufreq_policy *policy;
cpumask_t tmp_mask = work_cpumask;
for_each_cpu(cpu, tmp_mask) {
this_strombreaker = &per_cpu(strombreaker_info, cpu);
policy = this_strombreaker->cur_policy;

if (this_strombreaker->force_ramp_up) {
this_strombreaker->force_ramp_up = 0;
...
new_freq = cpufreq_strombreaker_calc_freq(cpu,policy);
...
static struct attribute * strombreaker_attributes[] = {
...
static struct attribute_group strombreaker_attr_group = {
.attrs = strombreaker_attributes,
.name = "strombreaker",
};

static int cpufreq_governor_strombreaker(struct cpufreq_policy *new_policy,
unsigned int event)
{
unsigned int cpu = new_policy->cpu;
int rc;
struct strombreaker_info_s *this_strombreaker = &per_cpu(strombreaker_info, cpu);
...
if (this_strombreaker->enable) /* Already enabled */
break;
...
rc = sysfs_create_group(&new_policy->kobj, &strombreaker_attr_group);
if (rc)
return rc;
pm_idle_old = pm_idle;
pm_idle = cpufreq_idle;

this_strombreaker->cur_policy = new_policy;
this_strombreaker->cur_policy->max = CONFIG_MSM_CPU_FREQ_ONDEMAND_MAX;
this_strombreaker->cur_policy->min = CONFIG_MSM_CPU_FREQ_ONDEMAND_MIN;
this_strombreaker->cur_policy->cur = CONFIG_MSM_CPU_FREQ_ONDEMAND_MAX;
this_strombreaker->enable = 1;

// notice no break here!

case CPUFREQ_GOV_LIMITS:
if (this_strombreaker->cur_policy->cur != new_policy->max)
__cpufreq_driver_target(new_policy, new_policy->max, CPUFREQ_RELATION_H);

break;

case CPUFREQ_GOV_STOP:
this_strombreaker->enable = 0;

if (atomic_dec_return(&active_count) > 1)
return 0;
sysfs_remove_group(&new_policy->kobj,
&strombreaker_attr_group);

pm_idle = pm_idle_old;
del_timer(&this_strombreaker->timer);
break;
}
...
static void strombreaker_suspend(int cpu, int suspend)
{
struct strombreaker_info_s *this_strombreaker = &per_cpu(strombreaker_info, smp_processor_id());
struct cpufreq_policy *policy = this_strombreaker->cur_policy;
unsigned int new_freq;

if (!this_strombreaker->enable || sleep_max_freq==0) // disable behavior for sleep_max_freq==0
return;
...
static void strombreaker_early_suspend(struct early_suspend *handler) {
int i;
suspended = 1;
for_each_online_cpu(i)
strombreaker_suspend(i,1);
}

static void strombreaker_late_resume(struct early_suspend *handler) {
int i;
suspended = 0;
for_each_online_cpu(i)
strombreaker_suspend(i,0);
}

static struct early_suspend strombreaker_power_suspend = {
.suspend = strombreaker_early_suspend,
.resume = strombreaker_late_resume,
};

static int __init cpufreq_strombreaker_init(void)
{	
unsigned int i;
struct strombreaker_info_s *this_strombreaker;
...
this_strombreaker = &per_cpu(strombreaker_info, i);
this_strombreaker->enable = 0;
this_strombreaker->force_ramp_up = 0;
this_strombreaker->time_in_idle = 0;
this_strombreaker->idle_exit_time = 0;
// intialize timer:
init_timer_deferrable(&this_strombreaker->timer);
this_strombreaker->timer.function = cpufreq_strombreaker_timer;
this_strombreaker->timer.data = i;
}

/* Scale up is high priority */
up_wq = create_rt_workqueue("kstrombreaker_up");
down_wq = create_workqueue("kstrombreaker_down");

INIT_WORK(&freq_scale_work, cpufreq_strombreaker_freq_change_time_work);

register_early_suspend(&strombreaker_power_suspend);

return cpufreq_register_governor(&cpufreq_gov_strombreaker);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_strombreaker
pure_initcall(cpufreq_strombreaker_init);
#else
module_init(cpufreq_strombreaker_init);
#endif

static void __exit cpufreq_strombreaker_exit(void)
{
cpufreq_unregister_governor(&cpufreq_gov_strombreaker);
destroy_workqueue(up_wq);
destroy_workqueue(down_wq);
}

module_exit(cpufreq_strombreaker_exit);

MODULE_AUTHOR ("UdyneOS");
MODULE_DESCRIPTION ("'cpufreq_strombreaker' - A smart cpufreq governor");
MODULE_LICENSE ("GPL");